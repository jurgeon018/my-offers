...
async def test_reindex_offers_master_and_payed_by_command_update_with_multiple_billings(runner, pg):
    now = datetime.now(pytz.utc)
    offer_id = 209194477
    master_user_id = 29437831
    new_master_user_id = 29437832
    ...
    await pg.execute(
        ...
        [
            1, 1, 1, master_user_id, offer_id, now, now, 1, False, now, now,
            2, 1, 1, new_master_user_id, offer_id, now, now, 2, False, now, now
        ]
    )
    ...
    assert row['master_user_id'] == new_master_user_id
    assert row['old_master_user_id'] == master_user_id
    assert row['payed_by'] == new_master_user_id
