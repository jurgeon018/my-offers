...
from my_offers.services.offers import get_filters_mobile_v2
from ._get_objects_models import get_object_models_with_pagination
from ._prepare_offers import prepare_offers


async def v2_get_my_offers_public(
        request: entities.MobileGetMyOffersRequestV2,
        realty_user_id: int
) -> entities.MobileGetMyOffersResponseV2:
    filters: Dict[str, Any] = await get_filters_mobile_v2(
    ...
    return entities.MobileGetMyOffersResponseV2(
        page=MobilePageInfo(
            limit=request.limit,
            offset=request.offset,
