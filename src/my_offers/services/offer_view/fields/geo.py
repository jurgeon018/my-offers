...
def prepare_geo(*, geo: Optional[Geo], geo_urls: Dict[set, str], jk_urls: Dict[int, str]) -> OfferGeo:
    if not geo:
        return OfferGeo()
...
def _get_underground(
    ...
    return Underground(
        search_url=urls.get((Type.underground, underground.id)),
...
def _get_address(
    ...
    for address in address_info:
        if address.type and address.full_name and address.type in ADDRESS_TYPES_MAP:
            result.append(
                Address(
                    search_url=urls.get((address.type, address.id)),
