...
async def test_get_offer(mocker):
    ...
    # act
    result = await get_offer(QaGetByIdRequest(offer_id=111, user_id=55, status_tab=OfferStatusTab.active))
...
async def test_get_offer__not_found__error(mocker):
    ...
    with pytest.raises(BrokenRulesException):
        await get_offer(QaGetByIdRequest(offer_id=111, user_id=55, status_tab=OfferStatusTab.active))
...
async def test_get_offer_view(mocker):
    ...
    # act
    result = await get_offer_view(QaGetByIdRequest(offer_id=111, user_id=55, status_tab=OfferStatusTab.active))

    # assert
    assert result == offer_view
    get_object_model_by_id_mock.assert_called_once_with(111)
    v2_get_offer_views_mock.assert_called_once_with(
...
async def test_get_offer_view__not_found__error(mocker):
    ...
    # act
    with pytest.raises(BrokenRulesException):
        await get_offer_view(QaGetByIdRequest(offer_id=111, user_id=55, status_tab=OfferStatusTab.active))
